...
Component({
  ...
  methods: {
    ...
    _trigger(name, value) {
      console.log(name, value, this.data.options[value].text)
      this.triggerEvent(name, {
        value,
        title: this.data.options[value].text,
      });
    },
    //设置高亮index并且触发事件
    ...
    isArrayObject(arg) {
      ...
      let flag = true
      arg.map(item => {
        flag = typeof item === "object" ? true : false
      })
      return flag
    },
  },
  ...
  ready: function() {
    ...
    if (!_isArrayObject) {
      const formatOptions = options.map(text => Object.assign({}, {
        text
      }));
      this.setData({
